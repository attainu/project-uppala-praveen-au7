...
export const fetchUser = ({username,password},cb) =>{
    return (dispatch) =>{
        ...
        axios.post("https://abc-app-123.herokuapp.com/app/v1/home/login",{username:username,password:password})
        .then(response=>{
...
export const addContact = ({id,email},cb) => {
    return (dispatch) => {
        ...
        axios.patch(`https://abc-app-123.herokuapp.com/app/v1/user/${id}?addContact=${email}`,{email:email},{headers:{'Authorization':`Bearer ${token}`}})
        .then(response=>{
...
export const removeContact = ({id,email},cb) => {
    return (dispatch) => {
        ...
        const token = getHomeToken()
        axios.patch(`https://abc-app-123.herokuapp.com/app/v1/user/${id}?removeContact=${email}`,{email:email},{headers:{'Authorization':`Bearer ${token}`}})
        .then(response=>{
            const homeToken = response.data.token
            if(response.data.success){
                let contacts = []
...
export const addNewMessage = (id,newMessage,cb) => {
    return (dispatch) => {
        ...
        axios.patch(`https://abc-app-123.herokuapp.com/app/v1/user/${decodedUser._id}?contact=${newMessage.receiver}&newMessage=${newMessage.message}`,{...newMessage},{
...
export const newMessageReceived = (newMessage) => {
    return (dispatch) => {
        ...
        try{
            ...
            createdUser.chats[newMessage.sender].push(newMessage)
            let user = {}
...
export const logoutUser = () =>{
    return (dispatch)=>{
        const token = getHomeToken()
        axios.patch('https://abc-app-123.herokuapp.com/app/v1/home/logout/',{token:token},{headers:{
