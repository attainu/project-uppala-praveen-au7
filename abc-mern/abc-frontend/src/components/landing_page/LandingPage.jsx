...
const Landing_page = (props) => {
   ...
   if(window.location.search.split("=")[0]==="?register"){
        ...
        if(obtainedToken){
            axios.post("http://localhost:5000/app/v1/home/register",{token:obtainedToken})
        .then(response=>{
    ...
    return (
            ...
            <Switch>
                <Route path="/" exact render={(props)=>{
                    ...
                    return <Landing_page {...props} />}} ><div style={message.length>0 || error.length>0?{display:"block"}:{display:"none"}} className={message.length>0?classes.verification_message:classes.verification_error}><h3 className={classes.message}>{notification}</h3></div></Route>
