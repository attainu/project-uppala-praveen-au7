...
const sendEmailVerificationLink = async (req,res,next) => {
    const msg = {
        ...
        <a href= "http://localhost:3000/?register=${req.token}">click on this link to verify your account</a>
        `,
      }
      sgMail
      .send(msg)
...
const sendResetPasswordLink = async (req,res,next) => {
    const msg = {
        ...
        <a href= "http://localhost:3000/resetPassword/${req.token}">click on this link to reset your password</a>
        `,
      }
      sgMail
      .send(msg)
